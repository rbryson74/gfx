...
static uint8_t __attribute__ ((coherent, aligned (32))) ScratchBuffer[SCRACH_BUFFER_SZ];

static uint32_t state;
static gfxBool useGPU=true;
static unsigned int vsyncCount = 0;
static gfxPixelBuffer pixelBuffer;
static gfxPixelBuffer scratchBuffer;
                             ...
                             gfxBlend blend)
{
    ...
    if ( useGPU )
    {
        ...
        gfxPixelBufferCreate(buf->size.width,
                    buf->size.height,
                    LCDC_DEFAULT_GFX_COLOR_MODE,
                    ScratchBuffer,
                    &scratchBuffer);
                
        memcpy((void*)scratchBuffer.pixels, (void*)buf->pixels, buf->buffer_length);
        ...
        _2dgpuGraphicsProcessor.blitBuffer(&scratchBuffer, &srcRect, &pixelBuffer, &destRect, blend );
    }
    else
    {
    	void* srcPtr;
